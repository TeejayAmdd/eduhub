          ...
          <div className="overflow-x-auto">
            <div className="min-w-[56rem]">
              ...
              <div className="mb-4 grid grid-cols-6 gap-2">
                <div className="rounded bg-muted p-3 text-sm font-semibold">
                  Time
                </div>
                {days.map((day) => (
                  <div
                    key={day}
                    className="rounded bg-muted p-3 text-center text-sm font-semibold"
                  >
                    {day}
                  </div>
                ))}
              ...
              {times.map((time) => (
                <div key={time} className="mb-2 grid grid-cols-6 gap-2">
                  <div className="flex items-center p-3 text-xs font-medium">
                    {time}
                  </div>
                  {days.map((day) => {
                    ...
                    return (
                      <div key={`${day}-${time}`} className="min-h-24">
                        {slot ? (
                          <Card className={`h-full min-w-0 p-3 ${slot.color}`}>
        ...
        <SectionCard title="Today's Classes">
          <div className="space-y-2">
            {["Monday", "Tuesday", "Wednesday"].map((day) => {
              const todayClasses = timeSlots.filter((slot) => slot.day === day);
              return (
                <div key={day}>
