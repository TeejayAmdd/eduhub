...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    // Capture ?open=ID before the async load clears the URL
    const openId = new URLSearchParams(window.location.search).get('open')
    if (openId) setPendingOpenId(Number(openId))
    refreshThreads().finally(() => setLoading(false))
  }, []) // eslint-disable-line react-hooks/exhaustive-deps

  // Once threads are loaded, auto-open the conversation from the URL param
  useEffect(() => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setInput('')
      // Update thread list so the last message preview refreshes
      refreshThreads()
    } catch (err) {
      console.error(err)
    } finally {
              ...
              {composeQuery.length < 2 ? (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">
                      {c.matric_number ?? c.department ?? c.email}
                    </p>
                  </div>
                </button>
              ))}
