      ...
      {Object.entries(grouped).map(([emoji, { count, mine }]) => (
        <button
          ...
          className={cn(
            ...
            mine ? 'bg-primary/10 border-primary/30 text-primary' : 'bg-muted border-border hover:bg-muted/80',
          )}
        >
          <span>{emoji}</span>
...
function MessageActions({
  ...
  return (
    <div
      ...
      <button
        onClick={async () => { try { onPin(await messages.pin(msg.id)) } catch {} }}
        className={cn(
...
function PinnedBanner({ pinned, onJump }: { pinned: Message[]; onJump: (id: number) => void }) {
    ...
    <div className="border-b border-border bg-muted/40 px-4 py-2">
      ...
      {expanded && (
        <div className="mt-2 space-y-1">
          {pinned.map((m) => (
            <button key={m.id} onClick={() => { onJump(m.id); setExpanded(false) }}
        ...
        <div className="p-4 border-b border-border space-y-3">
          ...
          <div className="relative">
            ...
            <Input placeholder="Filter conversations…" value={query}
              onChange={(e) => setQuery(e.target.value)} className="pl-8 h-9 text-sm" />
          </div>
        </div>
                ...
                <div className="space-y-4">
                  {conversation.map((m) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className="max-w-[78%] md:max-w-sm relative cursor-pointer select-none"
                      ...
                      <>
                        ...
                        <div className="absolute bottom-full left-0 mb-2 z-20 w-44 rounded-xl border bg-background shadow-lg overflow-hidden">
                          <button onClick={() => imageInputRef.current?.click()}
                            ...
                            <ImageIcon className="w-4 h-4 text-blue-500" /><span>Image</span>
                          </button>
                          ...
                          <button onClick={() => docInputRef.current?.click()}
                            className="flex w-full items-center gap-3 px-4 py-3 text-sm hover:bg-muted transition-colors border-t border-border">
                            <FileIcon className="w-4 h-4 text-blue-600" /><span>Document</span>
                          </button>
                        </div>
                      </>
                    )}
              ...
              {composeQuery.length < 2 ? (
                ...
                <button key={c.id}
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">{c.staff_number ?? c.department ?? 'Lecturer'}</p>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] px-4">
          ...
          <div className="relative z-10 w-full max-w-md rounded-2xl border bg-background shadow-xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-border">
              ...
              <Input
                autoFocus
                placeholder="Forward to… (search name or staff number)"
                value={forwardQuery}
